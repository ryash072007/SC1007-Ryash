...
def remove_duplicates_sorted_ll(ll):
    ...
    prev_current: ListNode = None
    current: ListNode = ll.head
    prev_val = None

    while current:
        change_prev_current = True
        if not prev_val:
            prev_val = current.item
        elif current.item == prev_val:
            prev_current.next = current.next
            change_prev_current = False
        else:
            prev_val = current.item
        if change_prev_current: prev_current = current
